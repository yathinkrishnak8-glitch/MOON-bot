...
class AIBossFightView(discord.ui.View):
    ...
    async def process_turn(self, interaction: discord.Interaction, action: str):
        ...
        self.chat_history.append({"role": "user", "content": f"I choose to: {action}. Describe the outcome under 100 words. At the end, output EXACTLY '[CONTINUE]', '[WIN]', or '[LOSE]'."})
        ...
        if "[WIN]" in reply.upper():
            ...
            embed = discord.Embed(title="🏆 BOSS SLAIN!", description=f"{clean}\n\n💰 **Rewards:** {reward:,} Coins!", color=discord.Color.gold())
            ...
            embed = discord.Embed(title="💀 YOU DIED", description=clean, color=discord.Color.dark_red())
            ...
            embed = discord.Embed(title="⚔️ BOSS FIGHT CONTINUES", description=clean, color=discord.Color.dark_theme())
    ...
    async def magic_button(self, interaction, button): await self.process_turn(interaction, "Cast offensive magic.")
    @discord.ui.button(label="Defend 🛡️", style=discord.ButtonStyle.success)
    async def defend_button(self, interaction, button): await self.process_turn(interaction, "Defend and block.")
    @discord.ui.button(label="Flee 🏃", style=discord.ButtonStyle.secondary)
    async def run_button(self, interaction, button):
        for child in self.children: child.disabled = True
        await interaction.response.edit_message(embed=discord.Embed(title="🏃 YOU FLED", description="You ran away.", color=discord.Color.light_grey()), view=self)
...
class MasterCommands(commands.Cog):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            embed = discord.Embed(description=f"⏳ **Hold up!** You are on cooldown. Try again in **{t}**.", color=discord.Color.red())
            if ctx.interaction and not ctx.interaction.response.is_done(): await ctx.interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        else: print(f"Ignored Error: {error}")
    ...
    async def shop_refresh_loop(self):
        master_items = [
            ...
            {"name": "Aura of the Conqueror", "price": 3000000, "desc": "Makes enemies faint."},
            {"name": "Philosopher's Stone", "price": 15000000, "desc": "Equivalent exchange is a myth."}
        ]
    ...
    async def ai_event_loop(self):
        ...
        if channel_id and self.client:
            ...
            if channel:
                try: 
                    reply = await self.ask_groq([{"role": "user", "content": "Generate a short engaging Discord event. No JSON."}])
    ...
    async def on_message_delete(self, message):
        if not message.author.bot: 
            snipes[message.channel.id] = {"content": message.content, "author": message.author.name, "avatar": message.author.avatar.url if message.author.avatar else message.author.default_avatar.url}
    ...
    async def on_message(self, message):
        ...
        for word in db["config"]["filterwords"]:
            if word in message.content.lower():
                try:
                    await message.delete(); await message.channel.send(embed=discord.Embed(description=f"⚠️ {message.author.mention}, that word is blacklisted!", color=discord.Color.red()), delete_after=5)
                except: pass
                return
    ...
    async def setaichannel(self, ctx): db["config"]["ai_channel"] = ctx.channel.id; save_db(db); await ctx.send(embed=discord.Embed(description=f"🤖 **AI Auto-Chat bound to** {ctx.channel.mention}", color=discord.Color.green()))

    @commands.hybrid_command(name="setcmdchannel", description="Locks normal commands to a channel.")
    @commands.has_permissions(administrator=True)
    async def setcmdchannel(self, ctx): db["config"]["cmd_channel"] = ctx.channel.id; save_db(db); await ctx.send(embed=discord.Embed(description=f"🔒 **Commands locked to** {ctx.channel.mention}", color=discord.Color.green()))

    @commands.hybrid_command(name="seteventchannel", description="Sets the hourly AI event channel.")
    @commands.has_permissions(administrator=True)
    async def seteventchannel(self, ctx): db["config"]["event_channel"] = ctx.channel.id; save_db(db); await ctx.send(embed=discord.Embed(description=f"🌟 **AI Events bound to** {ctx.channel.mention}", color=discord.Color.green()))
    ...
    async def unsetchannel(self, ctx, channel_type: str):
        ...
        if c not in valid: return await ctx.send(embed=discord.Embed(description="❌ **Invalid choice.** Use `ai`, `cmd`, or `event`.", color=discord.Color.red()))
        db["config"][valid[c]] = None; save_db(db)
        await ctx.send(embed=discord.Embed(description=f"✅ **Unbound the {c.upper()} channel.**", color=discord.Color.green()))
    ...
    async def deployserver(self, ctx):
        ...
        msg = await ctx.send(embed=discord.Embed(title="⚠️ CLEAN SLATE PROTOCOL", description="Are you absolutely sure you want to WIPE the server?", color=discord.Color.red()), view=view)
        await view.wait()
        if not view.value: return await msg.edit(embed=discord.Embed(description="🛑 Deployment Cancelled.", color=discord.Color.grey()), view=None)
        ...
        for r in roles_to_make:
            try: 
                ...
                cr[r["name"]] = await guild.create_role(name=r["name"], color=r["color"], permissions=perms, hoist=True)
    ...
    async def masterlist(self, ctx):
        embeds = []
        e1 = discord.Embed(title="🤖 Masterlist: Setup (Page 1/4)", color=discord.Color.blue()).add_field(name="/deployserver & /unsetchannel", value="Server setup.", inline=False).add_field(name="/aicommand", value="God-Mode AI python executor.", inline=False).add_field(name="/define, /bossfight, /lore", value="Interactive AI features.", inline=False)
    ...
    async def aicommand(self, ctx, *, instruction: str):
        await ctx.defer() # Crucial to prevent timeouts!
        if not self.client: return await ctx.send(embed=discord.Embed(description="🤖 **AI is offline.**", color=discord.Color.red()))
        prompt = f"""You are an omnipotent bot. Command: "{instruction}"
        Output a JSON array: 1. Reply: {{"action": "reply", "message": "text"}} 2. Execute Python: {{"action": "execute", "code": "await ctx.send('Task complete!')"}}
        ...
        try:
            ...
            for act in json.loads(clean):
                if act.get("action") == "reply": await ctx.send(embed=discord.Embed(description=f"🤖 {act.get('message')}", color=discord.Color.blurple()))
                    ...
                    msg = await ctx.send(embed=discord.Embed(description="⚡ **Executing dynamic Python code...**", color=discord.Color.orange()))
                    try:
                        ...
                        exec(w, exec_env); await exec_env['__ai_exec']()
                        await msg.edit(embed=discord.Embed(description="✅ **AI Code Execution Successful!**", color=discord.Color.green()))
                    except Exception as err: 
                        await msg.edit(embed=discord.Embed(description=f"⚠️ **AI Execution Failed:**\n```py\n{err}\n```", color=discord.Color.red()))
        except Exception as e: await ctx.send(embed=discord.Embed(description=f"❌ **Error parsing AI:** {e}", color=discord.Color.red()))
    ...
    async def bossfight(self, ctx):
        await ctx.defer() # Stops the Application Did Not Respond error
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI is offline. Dungeon Master is sleeping.", color=discord.Color.red()))
        ...
        try: scenario = await self.ask_groq(chat_history)
        except: return await ctx.send(embed=discord.Embed(description="❌ Failed to connect to AI.", color=discord.Color.red()))
    ...
    async def define(self, ctx, word: str): 
        ...
        reply = await self.ask_groq([{"role": "user", "content": f"Give a precise dictionary definition for: '{word}'"}])
    ...
    async def urban(self, ctx, word: str): 
        await ctx.defer()
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI offline.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "user", "content": f"Give an internet slang 'Urban Dictionary' definition for: '{word}'. Safe for work."}])
    ...
    async def forceevent(self, ctx): 
        ...
        await ctx.send(embed=discord.Embed(title="🌟 Forced Event", description=reply, color=discord.Color.blurple()))
    ...
    @commands.hybrid_command(name="roast_history", description="AI roasts a user based on history.")
    async def roast_history(self, ctx, member: discord.Member): 
        ...
        if not log: return await ctx.send(embed=discord.Embed(description=f"Not enough messages from {member.name}.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "user", "content": f"Roast this user:\n{log}"}])
        await ctx.send(embed=discord.Embed(title=f"🔥 Roast for {member.name}", description=reply, color=discord.Color.dark_orange()))

    @commands.hybrid_command(name="gothic_translate", description="Translates text into dark fantasy decree.")
    async def gothic_translate(self, ctx, *, text: str): 
        await ctx.defer()
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI offline.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "user", "content": f"Rewrite into gothic decree:\n{text}"}])
        await ctx.send(embed=discord.Embed(title="🦇 Gothic", description=reply, color=discord.Color.purple()))

    @commands.hybrid_command(name="lore", description="AI generates an epic backstory.")
    async def lore(self, ctx): 
        await ctx.defer()
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI offline.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "user", "content": f"Epic dark fantasy backstory for {ctx.guild.name}."}])
    ...
    async def debate(self, ctx, *, topic: str): 
        await ctx.defer()
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI offline.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "system", "content": "Take the opposite stance."}, {"role": "user", "content": topic}])
    ...
    async def tempban(self, ctx, member: discord.Member, days: int, *, reason: str="Temp Ban"): 
        ...
        await ctx.send(embed=discord.Embed(title="🔨 Temp-Banned", description=f"**{member.name}** banned for {days} days.\nReason: {reason}", color=discord.Color.red()))
        await asyncio.sleep(days * 86400); await ctx.guild.unban(member, reason="Tempban expired.")
    ...
    async def tempmute(self, ctx, member: discord.Member, minutes: int, *, reason: str="Temp Mute"): 
        await ctx.defer(); await member.timeout(timedelta(minutes=minutes), reason=reason)
        await ctx.send(embed=discord.Embed(title="🔇 Muted", description=f"**{member.name}** timed out for {minutes}m.", color=discord.Color.orange()))

    @commands.hybrid_command(name="slowmode", description="Sets slowmode.")
    @commands.has_permissions(manage_channels=True)
    async def slowmode(self, ctx, seconds: int): 
        await ctx.defer(); await ctx.channel.edit(slowmode_delay=seconds)
        await ctx.send(embed=discord.Embed(description=f"⏱️ **Slowmode set to {seconds}s.**", color=discord.Color.blue()))
    ...
    async def lockdown(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(description="🔒 **All channels locked down.**", color=discord.Color.dark_red()))
    ...
    async def unlockdown(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(description="✅ **Channels unlocked.**", color=discord.Color.dark_green()))
    ...
    async def snipe(self, ctx):
        ...
        embed = discord.Embed(description=data["content"], color=discord.Color.red()).set_author(name=data["author"], icon_url=data["avatar"])
    ...
    async def editsnipe(self, ctx):
        ...
        embed = discord.Embed(title="Message Edited", color=discord.Color.orange()).set_author(name=data["author"])
    ...
    async def purge(self, ctx, amount: int): 
        await ctx.defer()
        view = ConfirmView(ctx)
        msg = await ctx.send(embed=discord.Embed(description=f"⚠️ Delete {amount} messages?", color=discord.Color.orange()), view=view)
        await view.wait()
        if not view.value: return await msg.edit(embed=discord.Embed(description="🛑 Purge Cancelled.", color=discord.Color.grey()), view=None)
    ...
    async def nuke(self, ctx): 
        await ctx.defer()
        view = ConfirmView(ctx)
        msg = await ctx.send(embed=discord.Embed(title="☢️ TACTICAL NUKE", description="Are you sure?", color=discord.Color.red()), view=view)
        await view.wait()
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str="Caught lacking"): 
        await ctx.defer(); await member.kick(reason=reason); await ctx.send(embed=discord.Embed(description=f"👢 **{member.name} kicked.**", color=discord.Color.red()))

    @commands.hybrid_command(name="ban", description="Bans a user.")
    @commands.has_permissions(ban_members=True)
    async def ban(self, ctx, member: discord.Member, *, reason: str="Banned"): 
        await ctx.defer(); await member.ban(reason=reason); await ctx.send(embed=discord.Embed(description=f"🔨 **{member.name} permanently banned.**", color=discord.Color.dark_red()))
    ...
    async def warn(self, ctx, member: discord.Member, *, reason: str="Violation"): 
        await ctx.defer()
        db.setdefault("warns", {}).setdefault(str(member.id), []).append(reason); save_db(db)
        await ctx.send(embed=discord.Embed(description=f"⚠️ **{member.mention} warned.** Reason: {reason}", color=discord.Color.yellow()))

    @commands.hybrid_command(name="warnings", description="Shows a user's warnings.")
    async def warnings(self, ctx, member: discord.Member): 
        await ctx.defer()
        warns = db["warns"].get(str(member.id), [])
        if not warns: return await ctx.send(embed=discord.Embed(description=f"✅ **{member.name} is clean.**", color=discord.Color.green()))
    ...
    async def clearwarns(self, ctx, member: discord.Member): 
        await ctx.defer()
        db["warns"].pop(str(member.id), None); save_db(db)
    ...
    async def jail(self, ctx, member: discord.Member):
        await ctx.defer()
        db.setdefault("jailed", {})[str(member.id)] = [r.id for r in member.roles if r.id != ctx.guild.default_role.id]; save_db(db)
        ...
        if jr: 
            ...
            await ctx.send(embed=discord.Embed(description=f"⛓️ **{member.mention} locked up.**", color=discord.Color.dark_grey()))
        else: await ctx.send(embed=discord.Embed(description="⚠️ 'Jailed' role missing. Run `/deployserver`.", color=discord.Color.red()))
    ...
    async def unjail(self, ctx, member: discord.Member):
        await ctx.defer()
        jr = discord.utils.get(ctx.guild.roles, name="Jailed")
        if jr and jr in member.roles: await member.remove_roles(jr)
        for r_id in db.get("jailed", {}).pop(str(member.id), []):
            role = ctx.guild.get_role(r_id)
            if role: 
                try: await member.add_roles(role)
                except: pass
        save_db(db)
        await ctx.send(embed=discord.Embed(description=f"🔓 **{member.mention} made bail.**", color=discord.Color.green()))
    ...
    async def inventory(self, ctx): 
        await ctx.defer()
        items = db.get("inventory", {}).get(str(ctx.author.id), [])
    ...
    async def bal(self, ctx, m: discord.Member = None): 
        await ctx.defer()
        t = m or ctx.author; b = db["economy"].get(str(t.id), 0)
    ...
    async def daily(self, ctx): 
        await ctx.defer()
        db.setdefault("economy", {})[str(ctx.author.id)] = db["economy"].get(str(ctx.author.id), 0) + 500000; save_db(db)
    ...
    async def weekly(self, ctx): 
        await ctx.defer()
        db.setdefault("economy", {})[str(ctx.author.id)] = db["economy"].get(str(ctx.author.id), 0) + 5000000; save_db(db)
    ...
    async def slots(self, ctx, bet: int):
        ...
        await asyncio.sleep(0.8); embed.description = f"**[ {r1} | 🎰 | 🎰 ]**\n*Spinning...*"; await msg.edit(embed=embed)
        await asyncio.sleep(0.8); embed.description = f"**[ {r1} | {r2} | 🎰 ]**\n*Spinning...*"; await msg.edit(embed=embed)
        await asyncio.sleep(1.0); final = f"**[ {r1} | {r2} | {r3} ]**"
        ...
        save_db(db); await msg.edit(embed=embed)
    ...
    async def blackjack(self, ctx, bet: int):
        ...
        elif p > b or b > 21: db["economy"][u] += bet*2; embed.description = f"🃏 **Win!** You: {p}, Bot: {b}. Won **{bet*2:,}**!"; embed.set_image(url="[https://media1.tenor.com/m/9O3X3Y0O74cAAAAd/kakegurui-anime.gif](https://media1.tenor.com/m/9O3X3Y0O74cAAAAd/kakegurui-anime.gif)")
    ...
    async def coinflip(self, ctx, bet: int, c: str):
        ...
        embed = discord.Embed()
        if c.lower() == res: db["economy"][u] += bet*2; embed.description = f"🪙 **{res.title()}!** Won **{bet*2:,}**!"; embed.color = discord.Color.green(); embed.set_image(url="[https://media1.tenor.com/m/9O3X3Y0O74cAAAAd/kakegurui-anime.gif](https://media1.tenor.com/m/9O3X3Y0O74cAAAAd/kakegurui-anime.gif)")
        else: embed.description = f"🪙 **{res.title()}.** Lost."; embed.color = discord.Color.red()
        save_db(db); await ctx.send(embed=embed)

    @commands.hybrid_command(name="rich", description="View richest players.")
    async def rich(self, ctx): 
        await ctx.defer()
        se = sorted(db.get("economy", {}).items(), key=lambda x: x[1], reverse=True)
    ...
    @commands.hybrid_command(name="quest", description="Go on a dungeon quest.")
    @commands.cooldown(1, 3600, commands.BucketType.user)
    ...
    async def trade(self, ctx, m: discord.Member, item: str):
        await ctx.defer()
        u, t = str(ctx.author.id), str(m.id); i = item.lower()
    ...
    async def claimbounty(self, ctx, m: discord.Member):
        await ctx.defer()
        u, t = str(ctx.author.id), str(m.id); b = db.setdefault("bounties", {}).get(t, 0)
    ...
    async def rank(self, ctx, m: discord.Member = None): 
        await ctx.defer()
        t = m or ctx.author; l = db.setdefault("levels", {}).setdefault(str(t.id), {"xp": 0, "level": 1})
    ...
    async def leaderboard_levels(self, ctx): 
        await ctx.defer()
        sl = sorted(db.get("levels", {}).items(), key=lambda x: x[1]["level"], reverse=True)
    ...
    async def givexp(self, ctx, m: discord.Member, a: int): 
        await ctx.defer()
        db.setdefault("levels", {}).setdefault(str(m.id), {"xp": 0, "level": 1})["xp"] += a; save_db(db)
    ...
    async def removexp(self, ctx, m: discord.Member, a: int): 
        await ctx.defer()
        u = str(m.id); db["levels"][u]["xp"] = max(0, db.setdefault("levels", {}).get(u, {"xp":0})["xp"] - a); save_db(db)
    ...
    async def setlevel(self, ctx, m: discord.Member, l: int): 
        await ctx.defer()
        db.setdefault("levels", {}).setdefault(str(m.id), {"xp": 0, "level": 1})["level"] = l; save_db(db)
    ...
    async def rewards(self, ctx): 
        await ctx.defer()
        await ctx.send(embed=discord.Embed(title="🎁 Rewards", description="Lvl 10: Trusted\nLvl 50: Ronin\nLvl 100: God", color=discord.Color.green()))
    ...
    async def remindme(self, ctx, s: int, *, m: str): 
        await ctx.defer()
        await ctx.send(embed=discord.Embed(description=f"⏰ Set for {s}s.", color=discord.Color.blue()))
    ...
    async def afk(self, ctx, *, r="AFK"): 
        await ctx.defer()
        db.setdefault("afk", {})[str(ctx.author.id)] = r; save_db(db)
    ...
    async def avatar(self, ctx, m: discord.Member = None): await ctx.defer(); await ctx.send(embed=discord.Embed(color=discord.Color.blue()).set_image(url=(m or ctx.author).avatar.url))
    ...
    async def fakeban(self, ctx, m: discord.Member): 
        ...
        e.set_image(url="[https://media1.tenor.com/m/2Y4Z8v3_45wAAAAd/anime-ban.gif](https://media1.tenor.com/m/2Y4Z8v3_45wAAAAd/anime-ban.gif)"); await ctx.send(embed=e)
    ...
    async def pat(self, ctx, m: discord.Member): 
        ...
        e.set_image(url="[https://media1.tenor.com/m/8Y_2v_3_45wAAAAd/anime-pat.gif](https://media1.tenor.com/m/8Y_2v_3_45wAAAAd/anime-pat.gif)"); await ctx.send(embed=e)
    ...
    async def punch(self, ctx, m: discord.Member): 
        ...
        e.set_image(url="[https://media1.tenor.com/m/2Y4Z8v3_45wAAAAd/anime-punch.gif](https://media1.tenor.com/m/2Y4Z8v3_45wAAAAd/anime-punch.gif)"); await ctx.send(embed=e)
    ...
    async def bite(self, ctx, m: discord.Member): 
        ...
        e.set_image(url="[https://media1.tenor.com/m/ZzR7vT1wS4gAAAAd/anime-bite.gif](https://media1.tenor.com/m/ZzR7vT1wS4gAAAAd/anime-bite.gif)"); await ctx.send(embed=e)
    ...
    async def kiss(self, ctx, m: discord.Member): 
        ...
        e.set_image(url="[https://media1.tenor.com/m/2XyX9C4kG0QAAAAd/anime-kiss.gif](https://media1.tenor.com/m/2XyX9C4kG0QAAAAd/anime-kiss.gif)"); await ctx.send(embed=e)
    @commands.command(name="smug")
    async def smug(self, ctx): 
        e = discord.Embed(description=f"😏 **{ctx.author.name} is smug.**", color=discord.Color.purple())
        e.set_image(url="[https://media1.tenor.com/m/3_4X3Y0_45wAAAAd/anime-smug.gif](https://media1.tenor.com/m/3_4X3Y0_45wAAAAd/anime-smug.gif)"); await ctx.send(embed=e)
    @commands.command(name="cry")
    async def cry(self, ctx): 
        e = discord.Embed(description=f"😭 **{ctx.author.name} cries.**", color=discord.Color.blue())
        e.set_image(url="[https://media1.tenor.com/m/b-I3Jq7uU9wAAAAd/crying-anime.gif](https://media1.tenor.com/m/b-I3Jq7uU9wAAAAd/crying-anime.gif)"); await ctx.send(embed=e)
    ...
    async def domain_expansion(self, ctx): 
        ...
        e.set_image(url="[https://media1.tenor.com/m/7_3X2v2X14YAAAAd/domain-expansion.gif](https://media1.tenor.com/m/7_3X2v2X14YAAAAd/domain-expansion.gif)"); await ctx.send(embed=e)
    @commands.command(name="bankai")
    async def bankai(self, ctx): 
        e = discord.Embed(description=f"⚔️ **BANKAI!**", color=discord.Color.red())
        e.set_image(url="[https://media1.tenor.com/m/9O3X3Y0O74cAAAAd/bankai.gif](https://media1.tenor.com/m/9O3X3Y0O74cAAAAd/bankai.gif)"); await ctx.send(embed=e)
