...
xp_cooldown = set() # Prevents XP spamming
...
def get_default_db():
    return {
        ...
        "inventory": {}, "rep": {}, "bounties": {}, "current_shop": [], "gifs": DEFAULT_GIFS.copy()
    }
...
class BuyButton(discord.ui.Button):
    def __init__(self, item): super().__init__(label=f"Buy {item['name']} ({item['price']:,})", style=discord.ButtonStyle.success); self.item = item
...
class AIBossFightView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.ctx.author: await interaction.response.send_message("❌ You are not in this raid party!", ephemeral=True); return False
        return True
    async def process_turn(self, interaction: discord.Interaction, action: str):
        await interaction.response.defer()
        self.chat_history.append({"role": "user", "content": f"I act: {action}. Narrate the cinematic result of my move, how the boss reacts or counters, and the current battle status in 2 immersive paragraphs. Make it epic. At the absolute end, output EXACTLY '[CONTINUE]', '[WIN]', or '[LOSE]' based on the outcome."})
        try:
    ...
    async def attack_button(self, interaction, button): await self.process_turn(interaction, "Launch a devastating close-quarters combo attack.")
    @discord.ui.button(label="Cast Magic ✨", style=discord.ButtonStyle.primary)
    async def magic_button(self, interaction, button): await self.process_turn(interaction, "Channel mana and unleash an ultimate magic spell.")
    @discord.ui.button(label="Parry & Block 🛡️", style=discord.ButtonStyle.success)
    async def defend_button(self, interaction, button): await self.process_turn(interaction, "Wait for the boss to strike and attempt a perfect parry/block.")
    @discord.ui.button(label="Evade 💨", style=discord.ButtonStyle.secondary)
    async def dodge_button(self, interaction, button): await self.process_turn(interaction, "Use extreme speed to dodge the incoming attack and reposition.")
# ==========================================
...
class MasterCommands(commands.Cog):
    def __init__(self, bot):
        ...
        self.ai_event_loop.start(); self.shop_refresh_loop.start()

    def cog_unload(self): self.ai_event_loop.cancel(); self.shop_refresh_loop.cancel()
    ...
    async def on_message(self, message):
        ...
        # 1. PURE TEXT AI CHAT (ALL-CAPS FIX & MEMORY)
        is_dm = not message.guild
        is_ai_chan = message.guild and message.channel.id == db["config"].get("ai_channel")
        
        if (is_dm or is_ai_chan) and not message.content.startswith(('!', '/')) and self.client:
            async with message.channel.typing():
                try: 
                    # Fixed AI Prompt: Stop shouting in caps!
                    hist = [{"role": "system", "content": "You are habbibi mod (:, a sarcastic, funny, and highly intelligent Discord bot. Speak casually like a normal gamer. DO NOT use all caps unless absolutely necessary for emphasis. Do not shout."}]
        ...
        # 3. HARDCORE RPG LEVELING (Exponential Curve + 60s Cooldown)
        if uid not in db["levels"]: db["levels"][uid] = {"xp": 0, "level": 1}
        if uid not in xp_cooldown:
            xp_cooldown.add(uid)
            ...
            req_xp = int(150 * (current_lvl ** 1.5)) # Extreme leveling curve
    ...
    async def gif_add(self, ctx, category: str, url: str):
        await ctx.defer(ephemeral=True); c = category.lower()
    ...
    async def gif_remove(self, ctx, category: str, url: str):
        ...
        if c in db["gifs"] and url in db["gifs"][c]:
            ...
            await ctx.send(embed=discord.Embed(description=f"🗑️ **Removed!** `{c}` category now has {len(db['gifs'][c])} left.", color=discord.Color.red()))
    ...
    async def unsetchannel(self, ctx, channel_type: str):
        valid = {"ai": "ai_channel", "cmd": "cmd_channel", "event": "event_channel"}; c = channel_type.lower()
    ...
    async def vibecheck(self, ctx, member: discord.Member):
        ...
        reply = await self.ask_groq([{"role": "user", "content": f"Analyze the 'vibe' of this Discord user based on their recent messages. Are they chill, chaotic, toxic, or funny? Give a highly detailed, sarcastic vibe check report:\n\n{log}"}])
    ...
    async def roast_history(self, ctx, member: discord.Member): 
        ...
        reply = await self.ask_groq([{"role": "user", "content": f"Roast this user based on these messages:\n{log}"}])
    ...
    async def gothic_translate(self, ctx, *, text: str): 
        await ctx.defer()
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI offline.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "user", "content": f"Rewrite into dark gothic lore:\n{text}"}])
        await ctx.send(embed=discord.Embed(title="🦇 Gothic", description=reply, color=discord.Color.purple()))

    @commands.hybrid_command(name="lore", description="Server backstory.")
    ...
    @commands.hybrid_command(name="debate", description="Argue with the bot.")
    async def debate(self, ctx, *, topic: str): 
        await ctx.defer()
        if not self.client: return await ctx.send(embed=discord.Embed(description="❌ AI offline.", color=discord.Color.red()))
        reply = await self.ask_groq([{"role": "system", "content": "Argue aggressively against the user."}, {"role": "user", "content": topic}])
    ...
    async def heist(self, ctx): 
        ...
        embed = discord.Embed(title="🏦 BANK HEIST", description=f"You escaped with **{p:,} coins!**", color=discord.Color.gold())
    ...
    async def leaderboard_rep(self, ctx): 
        await ctx.defer(); sr = sorted(db.get("rep", {}).items(), key=lambda x: x[1], reverse=True)
