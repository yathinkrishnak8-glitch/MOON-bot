...
class MasterCommands(commands.Cog):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        # Unwrap errors caused by the command failing
        ...
        if isinstance(error, commands.CommandOnCooldown):
            ...
            # Catch-all to prevent "Thinking..." infinite freeze
            print(f"Command Error: {error}")
            err_msg = f"❌ **An internal error occurred:** `{str(error)}`"
            try:
                if ctx.interaction:
    ...
    async def bossfight(self, ctx):
        ...
        try:
            ...
            return await ctx.send(embed=discord.Embed(description=f"❌ Failed to connect to AI Dungeon Master: {e}", color=discord.Color.red()))
    ...
    async def roast_history(self, ctx, member: discord.Member): 
        ...
        if not chat_log: return await ctx.send(embed=discord.Embed(description=f"Not enough recent messages from {member.name}.", color=discord.Color.red()))
    ...
    async def domain_expansion(self, ctx): 
        embed = discord.Embed(description=f"🤞 **Domain Expansion!** {ctx.author.mention} has trapped everyone in their domain!", color=discord.Color.dark_blue())
